...
const SingleBlog = props => {
  ...
  const unlikePost = async blog => {
    if (
      props.blogs.singlePost.likes &&
      props.blogs.singlePost.likes.length !== 0
    ) {
  ...
  const postComment = async () => {
    if (comment && singleBlog._id) {
      ...
      let obj = {
        author: props.auth.user.lawfirm_user.id
  ...
  useEffect(() => {
    if (props.blogs.singlePost && props.profile) {
      props.blogActions.getBlogUserById(
        props.blogs.singlePost.author,
        props.blogs.singlePost.authorType
      );
          ...
          <div className="blog-main-content">
            ...
            <div className="featured-image">
              ...
              <div className="profilePicture">
                <img
                  src={
                    props.blogs.blogUser.profileImage
                      ? props.blogs.blogUser.profileImage.url
                      : Civil
                  }
                  alt="pretty_girl"
                />
              ...
              <div className="blog-body-and-comments">
                ...
                <div className="blogComments">
                    ...
                    {props.auth.isAuthenticated && (
                      <p>
                        <i
                          ...
                          onClick={() => {
                            liked
                              ? unlikePost(singleBlog)
                              : likePost(singleBlog);
                          }}
                        />

                        <span>
                          {singleBlog && singleBlog.likes
                    ...
                    {comments &&
                      comments.sort(handleSorting).map((item, index) => (
                        <li key={index}>
                          ...
                          <div>
                            <p>
                              {moment().diff(item.createdAt, "days") >= 1
                                ? `${moment().diff(
                                    item.createdAt,
                                    "days"
                                  )} days ago`
                                : moment().diff(item.createdAt, "hours") >= 1
                                ? `${moment().diff(
                                    item.createdAt,
                                    "hours"
                                  )} hours ago`
                                : moment().diff(item.createdAt, "minutes") > 1
                                ? `${moment().diff(
                                    item.createdAt,
                                    "minutes"
                                  )} minutes ago`
                  ...
                  <div className="inputContainer">
                    <img
                      src={props.auth.lawfirmUserProfile.profileImage.url}
                      alt="user_image"
                    />
                    <input
                      value={comment}
                ...
                return (
                  <div
                    ...
                    onClick={e => {
                      e.stopPropagation();
                      props.history.push(
                        `/lawfirm/blogs/${props.blogs.posts[index].id}`
                      );
                    }}
                  >
