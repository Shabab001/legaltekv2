...
const BlogCard = props => {
  ...
  useEffect(() => {
    ...
    const callBlogUser = async () => {
      let user = await props.blogActions.getBlogUserById(
        props.blog.author,
        props.blog.authorType
      );
      if (user) {
        setBlogUser(user);
      }
  ...
  const menu = props => (
    <Menu>
      ...
      <Menu.Item
        onClick={() =>
          props.history.push(`/lawyer/blogs/editblog/${props.item._id}`)
        }
      >
        <a>Edit post</a>
      </Menu.Item>
  ...
  async function deletePost(post, index) {
    ...
    let del = await deleteRequest(
      `${REACT_APP_API}/upload/files/${post.coverImage.id}`,
      "DELETE"
    );
    if (del) {
      console.log(del);
    ...
    <>
      {blogUser && (
        <div className="bcard-container">
          ...
          <div
            className="bcard-pic"
            onClick={() => props.history.push(`/lawyer/blogs/${props.blog.id}`)}
          >
          ...
          <div className="bcard-middle">
            <div className="bcard-middle-first">
              <div className="bcard-image">
                <div
                  className="bcard-round"
                  onClick={() => {
                    if (blogUser.lawfirmName) {
                      props.history.push(`/lawfirm-view/${blogUser.id}`);
                    } else {
                      props.history.push(`/lawyer-view/${blogUser.id}`);
                    }
                  }}
                >
                  <Tooltip title={`${blogUser.firstname} ${blogUser.lastname}`}>
                    <img
                      className="bcard-round-image"
                      src={blogUser.profileImage.url}
                      alt="propic"
                    />
                  </Tooltip>
                </div>
                <div
                  onClick={() =>
                    props.history.push(`/lawyer/blogs/${props.blog.id}`)
                  }
                  style={{ cursor: "pointer" }}
                >
                  <Tooltip title={props.blog.title}>
              ...
              >
                ...
                <p>
                  <span style={{ color: "black" }}>
                    {props.blog.likes ? props.blog.likes.length : 0}{" "}
                  </span>
                </p>
              </div>
            </div>
            <div className="bcard-desc">
              <p>{parse(props.blog.body.substring(0, 156))}</p>
            </div>
          </div>
