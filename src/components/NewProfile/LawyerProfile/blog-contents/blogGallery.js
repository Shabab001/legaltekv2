...
import { lawCats } from "../../BusinessProfile/lawyer modal/lawCats";
  ...
  useEffect(() => {
    if (props.blogs.posts) {
      ...
      if (criteria && criteria === "Newest") {
        setPosts(
          bPosts.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        );
        return;
      }
      if (criteria === "Like") {
    ...
    <>
      <div className="bgallery-main">
        ...
        <div className="bg-text">
          <p>
            {" "}
            Blogging enables you to reach millions of people that use the
            Internet. Blogging can help you promote your business by writing
            about important topics that are relevant to your audience
            establishes yourself as an authority in the space. It works as a
            method for attracting an audience because it provides something of
            value to them before asking for anything in return. Blogging can
            lead to other business/traffic generating opportunities. Here you
            can see a list of Blogs written by the people in your Firm.
          </p>
        </div>
          ...
          {posts && posts.length !== 0
            ? posts.map((item, index) => {
                if (searchTerm) {
                  ...
                  if (
                    criteria === "date" &&
                    moment(item.createdAt, dateFormat).isSame(searchTerm, "day")
                  ) {
                    return (
                      <BlogCard
                        blog={item}
